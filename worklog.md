...
Task ID: 45-bis
Agent: Super Z (agent principal)
Task: Vérification critique §26-bis — la chaîne de tranches Wake#N → Wake#N+1 ne doit PAS dépendre du navigateur ; continuation pilotée serveur/infrastructure, Neon source de vérité ; test réel d'arrêt/reprise entre deux phases ; moteur v2.1 intouché ; commit SANS push ni deploy.

Work Log:
- DIAGNOSTIC (avant correctif) : NON, la chaîne dépendait du navigateur — runWake exécute UNE tranche (budget 45 s) puis finalise partial (verrou libéré, curseur en Neon) ; le SEUL moteur de re-POST était la boucle for(;;) côté client de sync-wake-button.tsx (MAX_RESUMES=8) ; le polling frontend ne fait que des GET /api/sync/state (jamais de re-POST) → navigateur fermé = job partial figé jusqu'à un futur clic.
- CORRECTIF 1 — src/lib/sync/wake.ts : runWakeChainUntilDone(startedAtMs) — enchaîne les tranches CÔTÉ SERVEUR via runWake({triggerSource:'wake-chain'}) jusqu'à success/failed/stopped, avec murs SYNC_CHAIN_MAX_WALL_MS (défaut 240 000 ms < maxDuration 300 s — la synchro complète mesurée 215,7 s tient dans UNE invocation) et SYNC_CHAIN_MAX_LOOPS (défaut 8) ; arrêt propre sur failed (curseur conservé, relançable) et sur already_running/fresh.
- CORRECTIF 2 — src/app/api/sync/wake/route.ts : après une tranche partial, scheduleServerChain(t0) via after() (next/server) — le passage de tranche est piloté PAR LE SERVEUR dans la MÊME invocation ; catch → CHAIN_UNAVAILABLE (fallback frontend/cron) ; NOUVEAU handler GET = point d'entrée Vercel Cron (les crons Vercel émettent des GET) avec triggerSource='cron' — reprise SANS AUCUN utilisateur ; POST/GET répondent chain:'server'.
- CORRECTIF 3 — vercel.json (NOUVEAU) : cron /api/sync/wake toutes les 5 min — filet de sécurité infrastructure (reprise automatique de tout job partial/failed/orphelin sans utilisateur ; no-op 0 ESPN si frais).
- CORRECTIF 4 — takeover des verrous orphelins (> 6 min) : le curseur du job récupéré est DÉSORMAIS REPRIS (claimResumable + parse progress, resumedCount+1, log phase=recovered-resume) au lieu d'être abandonné à la prochaine invocation.
- Frontend sync-wake-button.tsx INCHANGÉ : sa boucle devient un fallback inoffensif (re-POST → already_running → suivi par polling) si after() était indisponible.
- TESTS RÉELS — scripts/test-wake-chain.ts (NOUVEAU, 4 phases, PG 17 persistant, ESPN RÉEL, VRAI serveur Next `next build` + `next start` VERCEL=1 — après constat que le sandbox tue les processus détachés entre commandes) :
  * inproc 35/35 : erreur de phase par injection RÉELLE (table Competition renommée pendant LIVE) → FAILED + curseur INTACT (121 ligues, stats cycle, espnCalls) + verrou libéré + relance réussie sur la MÊME ligne (resumedCount=2) ; orphelin 7 min → LOCK_RECOVERED + reprise du MÊME curseur (120→121, 14,3 s, pas de restart) ; orphelin 2 min → already_running (pas de takeover avant 6 min) ; nouveau wake utilisateur B → reprise automatique sans nouvelle ligne.
  * build 1/1 : next build production OK.
  * s7 22/22 — SCÉNARIO OBLIGATOIRE : POST réel (tranche 1 = 15 s) → partial avec LIVE fait + cycle entamé non achevé (arrêt ENTRE DEUX PHASES, curseur persisté) → « navigateur fermé » : ZÉRO appel HTTP jusqu'à la fin (observation Prisma seule) → SUCCESS serveur en 187 s, 16 tranches (resumedCount=15), 2 622 appels ESPN, 121/121 ligues, 4 phases done, verrou libéré, lastSyncAt à jour ; INVARIANTS échantillonnés à 200 ms : jamais 2 RUNNING simultanés, partiel JAMAIS success (0 violation), un seul curseur a avancé, aucune ligne wake créée après le clic ; logs : EXACTEMENT 1 STARTED triggerSource=wake vs 15 wake-chain, CHAIN_DONE ended=success, 0 CHAIN_ERROR/CHAIN_UNAVAILABLE (after() toujours disponible).
  * s89 18/18 : 5 POST simultanés → EXACTEMENT 1 gagnant (même runId P3 repris) + 4 already_running, achèvement SANS navigateur, invariant ≤ 1 RUNNING ; cron GET → reprise complète SANS utilisateur (P4, success) ; logs : 1 clic, ≥5 tranches wake-chain, 1 cron, 2 CHAIN_DONE success, 0 erreur.
- Vérifications finales : tsc --noEmit = 0 erreur src/ et scripts/ revenu à EXACTEMENT la baseline 85 (le nouveau script : 0 erreur) ; sha256sum -c baseline moteur = 5/5 OK ; scan secrets diff + fichiers touchés = 0 correspondance ; vercel.json neutre (crons seulement).

Stage Summary:
- RÉPONSE À LA QUESTION CRITIQUE : AVANT = NON (dépendance navigateur prouvée, boucle client) ; APRÈS correctif = OUI — la chaîne de 5-7 (voire 16) invocations continue SANS navigateur : after() enchaîne les tranches serveur dans la même invocation (mur 240 s < maxDuration 300 s ≥ 215,7 s mesurés), le cron GET */5 min reprend tout job interrompu sans utilisateur, le takeover > 6 min reprend le MÊME curseur.
- Neon = source de vérité confirmée par tests réels : curseur lu/écrit à chaque lot, reprise exacte (120/121 → 121), partiel jamais SUCCESS, erreur de phase sans perte de progression, verrou PostgreSQL (index unique partiel) inviolable (5 clics = 1 runner).
- Diff supplémentaire : wake.ts (+87 l. : chaîne serveur + takeover-curseur), route wake (after() + GET cron + chain:'server'), vercel.json (cron */5), test-wake-chain.ts (nouveau, 4 phases) ; frontend inchangé ; moteur 5/5 byte-identique.
- Commit local prêt ; NE PUSH PAS, NE DÉPLOIE PAS — attente du GO final.

---
Task ID: 46
Agent: Super Z (agent principal)
Task: GO pré-déploiement §26 — application migration Wake-on-Demand (miroir PG 17 local, Neon BLOQUÉ — credentials perdues au reboot, cf. risque documenté Task 28-b), vérifications complètes migration + wake réel + cron + continuation serveur ; moteur v2.1 et Option B intouchés ; commit SANS push ni deploy.

Work Log:
- BLOQUAGE NEON : ni .zscripts/.env.neon ni backups/.env.neon.copy (reboot conteneur — risque documenté Task 28-b matérialisé), repo.tar 00:12 sans ces fichiers, aucun env var, aucun processus vivant porteur → application Neon reportée jusqu'à transmission des credentials (procédure : DATABASE_URL="postgresql://…neon…" bun scripts/apply-wake-sync.ts). Tout le reste a été exécuté et prouvé sur miroir PG 17 local (port 5577) + ESPN RÉEL.
- BUG CORRIGÉ scripts/apply-wake-sync.ts (JAMAIS exécuté avant aujourd'hui) : la sonde anti-concurrence nettoyait son INSERT par tx.delete(...) APRÈS le rejet attendu du 2e INSERT — or une transaction PostgreSQL ayant échoué est ABORTED (25P02) → delete rejeté, script en échec. Correctif : exception sentinelle ProbeResult → ROLLBACK systématique (zéro résidu, aucun verrou orphelin possible même en crash).
- Miroir Neon (scripts/t46-mirror-seed.ts) : mire production-like (formes réelles du schéma : 3 competitions, 12 teams, 20 matchs, 10 résultats, 30 cotes, 2 snapshots §27 avec marchés/issues, 8 SyncJobRun worker status=NULL) + baseline MD5/comptage de 9 tables.
- Migration appliquée via le script Neon EXACT (idempotent, 2×) : 4 colonnes TEXT NULLABLE + index unique partiel + sonde verrou OK ; scripts/t46-verify-intact.ts → 9/9 tables IDENTIQUES (md5 bit-à-bit), 0 doublon verrou, historique worker 8/8 intact — migration purement additive.
- Répétition générale wake RÉELLE (scripts/t46-wake-rehearsal.ts, budget 15 s volontaire pour forcer les interruptions) : création (ligne verrouillée triggerSource=wake), 2e runWake → already_running MÊME runId, INSERT brut RUNNING → rejeté par l'index, tranche 1 partial verrou libéré curseur persisté, reprise MÊME runId resumedCount=1 leaguesDone en AVANCE (pas de restart), 31 tranches, 121/121 ligues, 1213 matchs, 2040 cotes, 2612 appels ESPN. 5 échecs de check = ARTEFACT attendu : budget 15 s < CONTEXT_MIN_MS (25 s) → la phase contexte ne peut jamais démarrer (boucle partial ; le mur/maxLoops de la chaîne la borne en production) — documenté dans le script.
- Complétion (scripts/t46-wake-complete.ts, budget production 45 s) : reprise du MÊME runId → phase contexte (standings=293, weather=85, 31,3 s) → SUCCESS : verrou libéré, 4/4 phases, 121/121, espnCalls cumulés 2773, stale=false liveStale=false, wake suivant = fresh 0 ESPN, 9/9 checks.
- Suite établie test-wake-sync.ts : 81/82 → 1 échec diagnostiqué = TEST environnement-dépendant (le picker S7 prend N'IMPORTE QUEL événement du jour ; celui-ci n'était pas encore commencé → le wake a CORRECTEMENT resynchronisé la ligne sur la vérité ESPN = SCHEDULED) ; assertion corrigée « statut = vérité ESPN actuelle (board re-consulté) » → re-run 82/82.
- Batterie continuation serveur test-wake-chain.ts RE-JOUÉE ENTIER : inproc 35/35 (erreur de phase par injection réelle → FAILED curseur intact → relance même ligne resumedCount=2 ; orphelin 7 min → takeover + reprise curseur 120→121 ; orphelin 2 min → already_running ; utilisateur B → reprise sans nouvelle ligne), build 1/1, s7 22/22 (1 clic → 17 tranches SERVEUR, 209 s, 2613 appels ESPN, ZÉRO HTTP après le clic, invariants 988 échantillons : jamais 2 RUNNING, partial jamais success, 1 seul curseur, aucune ligne post-clic), s89 18/18 (5 POST simultanés → 1 gagnant même curseur P3 ; cron GET → reprise SANS utilisateur P4 ; logs : 1 clic, ≥5 wake-chain, 1 cron, 2 CHAIN_DONE, 0 erreur).
- Option B intacte : test-option-b.ts 69/69 (trigger §20bis actif) ; test-prediction-freeze.ts 52/52 ; EXISTING_WORKER conservé (tick route + garde VERCEL 403 + logs SOURCE=EXISTING_WORKER — coexistence inoffensive par ingestion idempotente).
- Vérifications finales : moteur v2.1 sha256sum -c = 5/5 byte-identique ; tsc --noEmit = 0 erreur src/ + 0 erreur dans les fichiers nouveaux/modifiés (baseline HEAD mesurée par stash = 88, l'ancienne mention 85 = environnement conteneur précédent) ; scan secrets diff + nouveaux fichiers = 0 correspondance (seule URL : postgresql://postgres@127.0.0.1 — miroir local) ; next build production OK.

Stage Summary:
- Migration §26 prouvée additive et sûre sur miroir PG 17 identique au script Neon ; apply-wake-sync.ts corrigé (bug 25P02 de la sonde — première exécution réelle) et idempotent ; données existantes bit-identiques avant/après (9 tables).
- Wake réel bout-en-bout sur ESPN réel : création/progression/verrou/reprise multi-tranches/SUCCESS/idempotence fraîcheur — 82/82 suite + 9/9 complétion + 76/76 chaîne (dont scénario obligatoire SANS navigateur : 17 tranches serveur, invariants intacts).
- NEON EN ATTENTE DE CREDENTIALS : dès réception ( DATABASE_URL="postgresql://…" bun scripts/apply-wake-sync.ts ), l'application prend ~1 min ; puis re-vérifications Neon réelles (colonnes/index/doublons/état/wake) avant GO push/deploy. NE PUSH PAS, NE DÉPLOIE PAS.

---
Task ID: 47
Agent: Super Z (agent principal)
Task: GO « Neon réelle uniquement » — application de la migration Wake-on-Demand §26 sur la production Neon (credentials fournies par l'utilisateur, stockées hors Git), freeze §20bis, wake réel complet (création/verrou/curseur/reprise/SUCCESS/idempotence), vérifications 15 points, commit local SANS push ni deploy.

Work Log:
- Credentials sécurisées : .zscripts/.env.neon (chmod 600, couvert par .gitignore:78, git status propre) — DATABASE_URL Neon + GH_TOKEN ; aucune trace dans un fichier suivi (scan : 0 correspondance npg_/github_pat_/neon.tech dans le diff et les nouveaux scripts).
- Connexion Neon réelle : PostgreSQL 18.6 (aarch64), base neondb, SyncJobRun = 392 lignes ; colonnes §26 ABSENTES avant migration (état pré-migration confirmé — erreur 42703 attendue sur status) ; 21 tables (20 données + _prisma_migrations).
- BASELINE PRÉ-MIGRATION (scripts/t47-neon-baseline.ts) : 20 tables fingerprintées (comptage + MD5 avec listes de colonnes GELÉES — la comparaison post-migration ne peut pas être faussée par les 4 nouvelles colonnes) ; total 44 308 lignes ; artefact backups/t47-neon-baseline.json (hors Git) ; dernière activité worker 2026-09-16T00:03:24Z (~27 h) → aucune écriture concurrente pendant la fenêtre.
- MIGRATION (scripts/apply-wake-sync.ts, 1re exécution RÉELLE sur Neon — jamais exécuté avant T46-miroir) : 2 exécutions OK (idempotence prouvée) ; 4 colonnes TEXT NULLABLE status/triggerSource/runningLock/progress + index unique partiel SyncJobRun_running_lock_uidx ; sonde anti-concurrence : 2e INSERT RUNNING rejeté par PostgreSQL, transaction ROLLBACK-ée (zéro résidu).
- POST-MIGRATION (scripts/t47-neon-verify-post.ts) : 9/9 PASS — 20/20 tables BIT-À-BIT IDENTIQUES (0 modifiée, 0 supprimée), _prisma_migrations inchangée, 4/4 colonnes TEXT NULLABLE, index partiel présent, 0 doublon verrou, 0 résidu sonde, 0 verrou résiduel, 392 lignes SyncJobRun intactes (aucune nouvelle ligne), 10 triggers §20 inchangés.
- TRIGGERS : inventaire pré-migration = 10 triggers §20 (no_update/no_delete sur ForecastSnapshot, PredictionComponent, PredictionMarket, PredictionOutcome, PredictionSnapshot) ; freeze §20bis ABSENT de Neon → scripts/apply-prediction-freeze.ts exécuté ×2 (idempotent) → Prediction_freeze_update + Prediction_freeze_delete actifs ; sonde production-safe (scripts/t47-neon-freeze-probe.ts, transactions ROLLBACK-ées) : UPDATE figé rejeté + DELETE rejeté + ligne sonde intacte = 3/3 PASS ; TOTAL FINAL : 12 triggers.
- WAKE RÉEL sur Neon (scripts/t47-neon-wake.ts, budget PRODUCTION 45 s, aucune URL codée, garde neon.tech) : 43 PASS / 0 FAIL — runId unique cmu4ywcec0000ooue0h6mmguw ; 2e runWake pendant la tranche 1 → already_running (même runId) ; INSERT brut 2e RUNNING rejeté par l'index (sonde ROLLBACK-ée) ; tranche 1 partial (LIVE fait + 12/121, verrou libéré, curseur persisté) ; 7 reprises wake-chain sur le MÊME runId, curseur NON-RÉGRESSIF 12→30→54→66→84→108→121/121, resumedCount 0→7 ; SUCCESS : 4/4 phases (live/cycle/teamHistory/context), 121/121, verrou libéré, finishedAt renseigné, ligne wake unique, 0 verrou résiduel.
- MESURES du wake : 2 773 appels ESPN (compteur in-process) ; Match 7 270 → 7 337 (Δ+67 ; 190 matchs touchés = 67 créés + 123 maj) ; MatchResult 7 270 → 7 337 (187 resultsUpserted) ; OddsSnapshot 6 903 → 7 991 (Δ+1 088, toutes nouvelles lignes confirmées par createdAt) ; OddsOpenClose +327 ; StandingsSnapshot +969 ; WeatherSnapshot +87 ; Team +3 ; Total base 44 308 → 46 919 (Δ+2 611, APPEND-ONLY) ; durée du scénario 407,9 s (8 tranches ≈ 390 s de travail, budget 45 s/tranche) ; Prediction 3 504 → 3 504 INCHANGÉE (freeze + périmètre wake).
- SECOND WAKE frais : started:false reason:fresh, 0 appel ESPN ; getSyncState → stale=false liveStale=false, resumable=null, lastSyncAt=2026-09-17T03:32:37Z.
- SyncJobRun +3 lignes EXPLIQUÉES : 1 ligne §26 wake (status=success, triggerSource=wake, verrou libéré — startedAt rafraîchi à chaque reprise par design, wake.ts:436) + 2 lignes posthumes format-worker (status=NULL, phases live/context) écrites par les primitives partagées runLiveSync (espn-sync.ts:891) / runContextSync (context-sync.ts:457) — comportement PRÉ-EXISTANT, coexistence inoffensive documentée T45 ; horloge conteneur/Neon vérifiée (décalage 0,4 s).
- EXISTING_WORKER intact : logs SOURCE=EXISTING_WORKER (sync-job.ts:59) + garde VERCEL 403 (tick route) inchangés ; git diff tracked VIDE (aucun fichier suivi modifié — seuls 4 nouveaux scripts T47 non committés à ce stade).
...
Stage Summary:
- NEON RÉELLE MIGRÉE ET VALIDÉE : migration §26 purement additive appliquée (20/20 tables bit-à-bit intactes), idempotente, verrou anti-concurrence opérationnel en production (2e INSERT rejeté par PostgreSQL), freeze §20bis en place (12 triggers totaux), wake réel bout-en-bout SUCCESS (8 tranches, curseur 121/121, 2 773 ESPN, 2 611 lignes ajoutées en append-only), second wake = 0 ESPN, Prediction intacte.
- Le correctif server-driven Wake on Demand fonctionne sur la vraie infrastructure : reprise multi-tranches sur le MÊME runId via le curseur Neon, sans navigateur (7 reprises wake-chain consécutives prouvées ici en in-process, mécanisme identique à after()/cron validé en T45-bis/T46).
- Credentials dans .zscripts/.env.neon (hors Git) ; secrets = 0 dans le repo ; moteur 5/5 byte-identique ; EXISTING_WORKER conservé comme fallback.
- Commit local des 4 scripts T47 — NE PUSH PAS, NE DÉPLOIE PAS — attente du GO utilisateur pour push GitHub + deploy Vercel.

---
Task ID: 48
Agent: Super Z (agent principal)
Task: GO final — push GitHub + déploiement Vercel + vérifications production (migrate deploy, cron wake, sync/state, pas de worker, moteur intouché) + NOUVELLE FONCTIONNALITÉ PWA : bouton « Installer VOLTRIX » dans le Profil. Ne modifier que le nécessaire pour ces deux objectifs.

Work Log:
- Pré-push : 0 credential réel dans le commit 1935972 (valeurs réelles scannées : mot de passe Neon, token GitHub, hôte) ; .zscripts/.env.neon toujours couvert par .gitignore:78 ; migration SQL §26 vérifiée idempotente (IF NOT EXISTS partout → migrate deploy sûr sur Neon où les objets existent déjà).
- PUSH #1 (1935972, 8 commits 039ee22→1935972) : auth par askpass hors Git (backups/askpass.sh, chmod 700, token lu depuis .env.neon — jamais en ligne de commande ni en config Git) ; SHA distant == SHA local vérifié.
- DÉPLOIEMENT #1 ÉCHOUÉ (statut GitHub « Deployment failed. » ~40 s après push, lien vercel.link → docs/cron-jobs/usage-and-pricing = preuve) : cause #1 = cron vercel.json */5 * * * * dépasse la limite plan (Hobby = 1×/jour). Correctif 890b8a4 : cron → 0 5 * * *.
- DÉPLOIEMENT #2 ÉCHOUÉ (~40 s, ID dpl_Ge19Ke5jMEWc65kLtYaoJgG99tfX) : cause #2 isolée = maxDuration=300 sur /api/sync/wake dépasse le plafond fonctions du plan (connu-sûr = 120 s d'après le dernier succès 039ee22 avec routes 120). Build local complet (prisma generate + migrate deploy + next build + cp) PASSAIT — le code était sain, pur problème de validation plan.
- Correctif c0dec17 : maxDuration 300→120 + mur chaîne serveur SYNC_CHAIN_MAX_WALL_MS 240000→110000 avec vérification ANTICIPÉE dans la boucle after() (n'amorce une tranche que si elapsed + budget 45 s + marge 60 s phases monolithiques ≤ mur) → finalisation TOUJOURS propre (partial + verrou libéré + curseur conservé), jamais de kill plateforme ; complétion sur 2-3 invocations via cron GET / boucle bouton / prochain wake ; upgrade plan documenté dans le code (env SYNC_CHAIN_MAX_WALL_MS=240000 + maxDuration 300).
- DÉPLOIEMENT #3 RÉUSSI (c0dec17, ~2 min) : sonde GET /api/sync/wake 404→200 ; un wake s'est déclenché en prod dès la mise en ligne et a fini SUCCESS (stale=false, liveStale géré, running=null, resumable=null, hasLiveMatches=true).
- Vérifications production : _prisma_migrations contient 20250917000000_wake_sync_state (enregistrée, steps:1 — migrate deploy OK) ; 2 jobs §26 status=success ; 0 verrou actif ; POST+GET /api/sync/tick → 403/403 (aucun worker continu sur Vercel) ; GET / + /api/matches + /api/performance + /api/forecasts/week → 200 (zéro régression) ; GET /api/sync/state → JSON complet.
- PWA (cade33e) : infrastructure préexistante découverte et conservée (manifest.webmanifest, sw.js v4→v5 bumpé selon sa propre convention, 5 icônes PNG validées 180/192/512+maskable, registration SW dans layout) ; NOUVEAU src/components/voltrix/pwa-install-button.tsx (client, 0 dépendance) : capture beforeinstallprompt → prompt() natif Android/Chromium ; détection déjà-installée (display-mode: standalone + navigator.standalone + listener appinstalled) → état « Application installée » ; iOS/iPadOS → « Télécharger l'application » + instructions Safari (Partager → Sur l'écran d'accueil → Ajouter) ; repli manuel adapté Android (menu ⋮) et desktop (icône ⊕ barre d'adresse) ; inséré dans ProfileTab (page.tsx) après le lien Précision ; manifest name/short_name → VOLTRIX ; layout applicationName + appleWebApp.title → VOLTRIX (titre d'écran d'accueil iOS).
- Vérifications PWA locales (next build + next start + navigateur réel) : SW enregistré + ACTIVÉ scope / ; manifest servi et valide (name VOLTRIX, standalone, start /, 4 icônes, theme) ; beforeinstallprompt CAPTURÉ (label « Installer VOLTRIX » = critères d'installabilité Chromium satisfaits) ; clic prompt() sans erreur ; émulation iPhone 14 → branche iOS avec instructions complètes ; hors-ligne → app shell servie depuis le cache SW (repli network-first navigation) ; tsc src/ 0 erreur.
- DÉPLOIEMENT #4 RÉUSSI (cade33e, ~1,5 min) : SHA distant == local ; manifest prod = VOLTRIX/standalone/4 icônes ; sw.js = voltrix-v5 ; 5 icônes → 200 image/png ; HTML prod : link manifest + meta apple VOLTRIX + script registration SW ; bouton présent dans l'onglet Profil (montage client après hydratation — vérifié au navigateur réel sur la PROD : installCard=true, label « Installer VOLTRIX » = beforeinstallprompt déclenché en production).
- Finaux : moteur v2.1 sha256 5/5 byte-identique ; secrets = 0 dans les 3 nouveaux commits ; arbre propre ; diff minimal (5 fichiers PWA + 2 commits config déploiement).

Stage Summary:
- VOLTRIX est DÉPLOYÉ EN PRODUCTION sur https://voltrixbet.vercel.app au SHA cade33e (GitHub == Vercel) avec le système Wake-on-Demand §26 opérationnel (wake réel SUCCESS dès la mise en ligne), le cron quotidien de reprise, le freeze §20bis et 12 triggers actifs sur Neon.
- DEUX LIMITES PLAN Vercel découvertes et contournées (documentées dans le code + chemin d'upgrade) : crons limités à 1×/jour (Hobby) et fonctions plafonnées à 120 s — la sync complète s'étale sur 2-3 invocations serveur enchaînées (curseur Neon, zéro perte), le navigateur n'est jamais requis.
- PWA livrée : VOLTRIX s'installe comme une vraie app (bouton Profil, natif Android, instructions iOS, état déjà-installée), même app/même URL/standalone, zéro dépendance ajoutée, zéro régression, moteur v2.1 byte-identique.
- Auth GitHub par askpass hors Git ; token GH stocké uniquement dans .zscripts/.env.neon (ignoré) ; aucun secret dans aucun commit.
...
Work Log:
- Repo re-cloné depuis GitHub (alamkevine226ascii-png/VOLTRIX-bet-, HEAD 7aee03a, arbre propre — GO 1/GO 2 déjà intégrés et commités par Task 48 : PwaInstallButton + chargement progressif runId présents, zéro diff dessus pendant Task 49).
- Diagnostic Neon LECTURE SEULE confirmé : dernier ForecastJobRun 15/09 16:58 UTC (phase=scan, finishedAt=NULL — worker tué en pleine exécution), 101 lignes au total, 934 snapshots v2.1 tous version=1, dernier snapshot 15/09 16:58:06 ; backlog = 139 matchs futurs (fenêtre 21 j, statuts SCHEDULED) sans AUCUN snapshot dont 2 < 48 h ; 12 triggers §20/§20bis actifs ; SyncJobRun_running_lock_uidx présent ; ForecastJobRun sans colonnes d'état. Les PredictionSnapshot postérieurs au 15/09 = artefacts de tests (modelVersion v2.1-test). Cause racine du沉默 : /api/forecasts/tick → 403 sous VERCEL + ensureForecastLoop no-op → plus AUCUN fabriquant de snapshot en production depuis le 15/09 alors que le Wake sync continuait d'alimenter Neon.
- NOUVEAU src/lib/forecast/wake.ts (miroir src/lib/sync/wake.ts) : détection (2 SELECT Neon, 0 ESPN — matchs kickoff futur < horizon 14 j, statut SCHEDULED/PRE, SANS aucun snapshot §5, tri kickoff asc, résolution leagueCode via Competition), verrou PostgreSQL (claimNewRun/claimResumable/finalizeRun + index unique partiel + takeover verrou orphelin 6 min), tranches budget 45 s (FORECAST_WAKE_BUDGET_MS, surchargeable), curseur progress.processed dans ForecastJobRun, quadruple garde kickoff (détection futur + lead 60 s + analysis.status==='pre' + §6 buildSnapshotDraft), INSERT-only via chemin production inchangé (analyzeMatch → buildSnapshotDraft → persistGenericSnapshot), P2002 [matchId,version] → skippedAlreadySnapshot (jamais 2 snapshots), chaîne serveur runForecastWakeChainUntilDone (mur 110 s, overshoot 20 s, grain fin), overview diagnostic.
- NOUVEAU src/app/api/forecasts/wake/route.ts (miroir /api/sync/wake) : POST (manuel) + GET (cron Vercel), maxDuration=120, chaîne after() server-driven — le navigateur n'est jamais requis.
- src/app/api/sync/wake/route.ts : diff STRICTEMENT additif (47 +, 0 logique modifiée) — après un succès de sync (direct OU fin de chaîne after()), déclenchement post-sync runForecastWakeChainUntilDone muré par le même t0 (jamais de dépassement maxDuration).
- prisma/schema.prisma : ForecastJobRun + 4 colonnes NULLABLES (status, triggerSource, runningLock, progress) — rétrocompatibles avec les 101 lignes worker historiques (NULL).
- NOUVELLE migration prisma/migrations/20260917000100_forecast_job_lock/migration.sql : additive + idempotente (IF NOT EXISTS, miroir exact de 20250917000000_wake_sync_state appliquée en prod sans incident).
- vercel.json : 2e cron quotidien /api/forecasts/wake à 05:30 UTC (limite Hobby = 2 crons 1×/jour — respectée).
- NOUVEAU scripts/apply-forecast-lock.ts (miroir apply-wake-sync.ts : colonnes + index + sonde verrou ROLLBACK-ée) — À EXÉCUTER SUR NEON UNIQUEMENT après GO utilisateur.
- NOUVEAU scripts/test-forecast-wake.ts (suite complète, PG17 éphémère, ESPN stubbé fixtures déterministes, port libre garanti, 10 sections A-K, 68 vérifications) + scripts/build-validate-local.ts (prisma migrate deploy ×2 + next build contre PG local) + scripts/smoke-routes-forecast-wake.ts (9 vérifications).
- Tests exécutés : test-forecast-wake 68/68 ✓ ; suites existantes : test-wake-sync 82/82 ✓ (ESPN réel), test-wake-chain inproc 35/35 ✓, test-option-b 69/69 ✓, test-performance-integrity 31/31 ✓, test-prediction-freeze 52/52 ✓ ; test-prediction-immutability + test-forecast-integrity = ÉCHEC PRÉ-EXISTANT reproduit sur HEAD propre (suites ère SQLite, incompatibles schema postgres + DIRECT_URL absent — indépendant de Task 49) ; test-forecast-weekly-cycle non exécuté (exige serveur dev + miroir DB locale — surfaces non touchées par le diff : /api/forecasts/week, week.ts, export/report inchangés).
- Build validation : prisma migrate deploy applique les 3 migrations (dont la nouvelle) + re-run no-op + 4 colonnes/index vérifiés + next build complet OK (/api/forecasts/wake enregistrée).
- Moteur v2.1 byte-identique 5/5 (sha256 baseline 039ee22 : prediction.ts, analyze.ts, espn.ts, market-odds.ts, model-version.ts) — vérifié AVANT et APRÈS toutes les modifications.
- .gitignore : +.tmp-pg/ +.tmp/ (binaires PG17 locaux, 100+ Mo — jamais commitables).
- AUCUNE écriture Neon, AUCUN commit, AUCUN push, AUCUN déploiement (attente GO explicite utilisateur conformément à GO 4B).

Stage Summary:
- Pipeline Forecast Wake implémenté et validé localement : Wake sync → ESPN→Neon → détection (2 SELECT, 0 ESPN) → Forecast Wake → moteur v2.1 inchangé → ForecastSnapshot immuable. Backlog 139 matchs absorbable par tranches (≈ 139 analyses × 3-10 s étalées sur invocations enchaînées after(), reprise par cron 05:30 UTC + post-sync + manuel).
